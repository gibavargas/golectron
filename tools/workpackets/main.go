...
func buildObjectivePackets(statuses, areas, ids map[string]bool) []packet {
	objectivePackets := []packet{
		...
		},
		{
			...
			Notes:       "Latest tracked duration_median_ms ratio is 0.7685 on the scoped runtime path; the objective requires electron_go_over_electron <= 0.5 with full runtime parity still intact.",
			E2ERequired: true,
			Acceptance: []string{
